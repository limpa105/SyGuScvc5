...
TypeNode blg = SygusUtils::getSygusBlockingGeneratorType(e);
...
if (!blk.isNull())
{
  ...
  Trace("sygus-block") << "SE::init setBlockingGrammarType to " << blk << "\n";
}
else
{
...
if (!blg.isNull())
{
  auto* sec = dynamic_cast<SygusEnumeratorCallback*>(d_sec);
  Assert(sec != nullptr);
  sec->setBlockingGeneratorGrammarType(blg);
  Trace("sygus-block") << "SE::init setBlockingGeneratorGrammarType to " << blg
                       << "\n";
}
else
{
  Trace("sygus-block") << "SE::init no blockingGeneratorType\n";
}
...
void SygusEnumerator::updateBlockingGrammarTypes(TypeNode blk, TypeNode blg)
{
 if (!blk.isNull())
{
  SygusUtils::setSygusBlockingType(d_enum, blk);
}
if (!blg.isNull())
{
  SygusUtils::setSygusBlockingGeneratorType(d_enum, blg);
}
...
if (blk.isNull())
{
   Trace("sygus-block") << "OH NOOO BLOCKLINGF GRAMMAR IS NULL\n";
}

if (!blk.isNull())
{
  sec->setBlockingGrammarType(blk);
  Trace("sygus-block") << "SE::updateBlockingGrammarTypes updated blk to "
                       << blk << "\n";
}
...
{
  sec->setBlockingGeneratorGrammarType(blg);
  Trace("sygus-block") << "SE::updateBlockingGrammarTypes updated blg to "
                       << blg << "\n";
}
if (!blk.isNull() && blk.isDatatype())
{
  const DType& dt = blk.getDType();
  Trace("sygus-block") << "update blk=" << blk
                       << " numConstructors=" << dt.getNumConstructors() << "\n";
  for (size_t i = 0, ncons = dt.getNumConstructors(); i < ncons; i++)
  {
    Trace("sygus-block") << "  blk cons[" << i << "] = "
                         << dt[i].getSygusOp() << "\n";
  }
}
}
